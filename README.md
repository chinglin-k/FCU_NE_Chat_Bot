...
- 🛡️ **多層安全防護**：
  ...
  * 後端例外一律回傳固定錯誤代碼，不外洩原始例外訊息（CWE-209 防護）
...
| Git 歷史機密殘留 | 舊版 commit 曾寫死真實 Spreadsheet ID；已用 `git-filter-repo` 於獨立環境清除並驗證，實際覆蓋 GitHub 遠端歷史需專案擁有者手動執行 force push（見上方 WARNING 區塊） |
